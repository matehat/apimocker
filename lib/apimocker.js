...
apiMocker.setSwitchOptions = function(options, req) {
  ...
  switches.forEach(function(s){
    ...
    if (switchParamValue) {
      switchFileCmps.push(s + "=" + switchParamValue);
    }
  });

  switchFilePrefix = switchFileCmps.join("&");
...
apiMocker.start = function (port) {
  ...
  apiMocker.expressInstance = apiMocker.express.listen(port, apiMocker.options.host);
